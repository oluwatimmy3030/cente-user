...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-30 border-b border-border/70 bg-background/92 backdrop-blur-xl"><div className="mx-auto grid h-18 max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center px-4 sm:px-7"><div className="lg:hidden"><Brand /></div><div className="hidden min-w-0 lg:block"><p className="text-xs text-muted-foreground">CENTE user account</p><p className="truncate text-sm font-medium">Your wealth, in one place</p></div><div className="flex shrink-0 items-center gap-2"><Button variant="ghost" size="icon" aria-label="Support" onClick={() => setSupport(true)}><CircleHelp /></Button><Button asChild variant="ghost" size="icon" className="relative"><Link to="/notifications" aria-label="Notifications"><Bell />{unread>0&&<span className="absolute right-1 top-1 size-2 rounded-full bg-primary"/>}</Link></Button><Link to="/profile" className="grid size-10 place-items-center rounded-full bg-primary text-sm font-semibold text-primary-foreground shadow-gold">SA</Link></div></div></header>
